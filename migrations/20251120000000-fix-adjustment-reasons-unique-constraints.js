...
/**
 * Fix Adjustment Reasons Unique Constraints for Multi-Tenant Support
 * 
 * This migration fixes unique constraints for adjustment_reasons:
 * 1. adjustment_reasons.code - should be unique per company, not globally
 * 2. adjustment_reasons.name - should be unique per company (NEW)
 * 
 * For each field:
 * - Drops old global unique constraints/indexes
 * - Ensures composite unique index with companyId is in place
 * 
 * This allows different companies to have the same adjustment reason codes and names.
 */
...
const TABLES_TO_FIX = [
  {
    ...
    fieldName: 'code',
    indexName: 'adjustment_reasons_code_companyId_unique'
  },
  {
    tableName: 'adjustment_reasons',
    fieldName: 'name',
    indexName: 'adjustment_reasons_name_companyId_unique'
  }
];
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      console.log('🔄 Fixing adjustment_reasons unique constraints for multi-tenant support...\n');
      ...
      console.log('\n✅ Successfully fixed adjustment_reasons unique constraints for multi-tenant support\n');
    } catch (error) {
      await transaction.rollback();
      console.error('❌ Error fixing adjustment_reasons unique constraints:', error);
      throw error;
    }
  },
