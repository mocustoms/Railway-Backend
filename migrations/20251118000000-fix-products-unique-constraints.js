...
/**
 * Fix Products Unique Constraints for Multi-Tenant Support
 * 
 * This migration fixes unique constraints for products:
 * 1. products.code - should be unique per company, not globally
 * 2. products.barcode - should be unique per company, not globally
 * 
 * For each field:
 * - Drops old global unique constraints/indexes
 * - Ensures composite unique index with companyId is in place
 * 
 * This allows different companies to have the same product codes and barcodes.
 */
...
const TABLES_TO_FIX = [
  {
    tableName: 'products',
    fieldName: 'code',
    indexName: 'products_code_companyId_unique'
  },
  {
    tableName: 'products',
    fieldName: 'barcode',
    indexName: 'products_barcode_companyId_unique'
  }
];
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    try {
      console.log('🔄 Fixing products unique constraints for multi-tenant support...\n');
      ...
      for (const table of TABLES_TO_FIX) {
        ...
        const fieldNameQuoted = table.fieldName === 'barcode' ? 'barcode' : `"${table.fieldName}"`;
        ...
        if (duplicates.length > 0) {
          ...
          if (indexExists.length === 0) {
            ...
            // For barcode, add WHERE clause to handle NULLs
            if (table.fieldName === 'barcode') {
              await queryInterface.sequelize.query(
                `CREATE UNIQUE INDEX "${table.indexName}" 
                 ON ${table.tableName} (${fieldNameQuoted}, "companyId") 
                 WHERE ${fieldNameQuoted} IS NOT NULL;`,
                { transaction }
              );
            } else {
      ...
      console.log('\n✅ Successfully fixed products unique constraints for multi-tenant support\n');
    } catch (error) {
      await transaction.rollback();
      console.error('❌ Error fixing products unique constraints:', error);
      throw error;
    }
  },
