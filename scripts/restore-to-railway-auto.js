...
/**
 * Restore Database Backup to Railway (Auto Mode - No Prompts)
 * 
 * Non-interactive version that automatically confirms all prompts
 * Usage: node scripts/restore-to-railway-auto.js <backup-file> <railway-database-url> [--clear-data]
 */
...
async function main() {
  ...
  if (!backupFile) {
    ...
    console.error('');
    console.error('Usage: node scripts/restore-to-railway-auto.js <backup-file> <railway-database-url> [--clear-data]');
    console.error('');
    console.error('Example:');
    console.error('  node scripts/restore-to-railway-auto.js backups/local-backup-2025-11-17.sql "postgresql://user:pass@host:port/db"');
    console.error('  node scripts/restore-to-railway-auto.js backups/local-backup-2025-11-17.sql "postgresql://user:pass@host:port/db" --clear-data');
    process.exit(1);
  }
  ...
  const railwayDbUrl = process.argv[3];

  if (!railwayDbUrl) {
    console.error('❌ Error: Railway DATABASE_URL is required');
    console.error('');
    console.error('Usage: node scripts/restore-to-railway-auto.js <backup-file> <railway-database-url>');
    process.exit(1);
  }
  ...
  console.log('');
  console.log('🔄 RESTORING DATABASE TO RAILWAY (AUTO MODE)');
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        if (executed % 50 === 0 || executed === total - failed) {
        ...
        if (failed <= 5 && !isIgnorable) {
          console.error(`\n⚠️  Error on statement ${i + 1}: ${error.message.substring(0, 200)}`);
        }
      }
    }
